define('common/module/financialsBar/financialsSmlBar.tpl', function(require, exports, module) {
  ...
  if(type == "regular"/*定期理财*/){
  	...
  	if(isNewerNotEnd){
  		...
  		opt.topRightTag = '<img class="financials-item-tag" src="' + '/static/common/module/financialsBar/financials_newer.png' + '">';
  	}else{
  	...
  	opt.bdText = '<a href="/product.html?id=' + bdid + '" class="wbtn wtbn-nothing">' + title + '</a>';
        ...
        _$out_.push('<div class="financials-item small" data-id="', bdid, '">', opt.topRightTag, '<h3>', opt.tTitle, '</h3><div class="financials-item-timelimit">', opt.tTitleExtra, '</div><div class="financials-item-interestrate">', opt.annualrateText, '</div><div class="financials-item-interestrate-a">预期年化收益率</div><div class="financials-item-intro-title">', opt.bdText, '</div>');
         if(type == "regular"){ 
  	var timeInfo = {
  		nowDate: nowDate,
  	...
  	curInfos.linkUrl = (dqUrl[productType] || '/nyb.html?id=') + bdid;
        ...
        _$out_.push('<div class="financials-item-intro">', curInfos.tipText, '</div>', curInfos.progressDom, '<div class="financials-item-operate">');
         if(curInfos.isWaiting){ 
        _$out_.push('<a href="', curInfos.linkUrl, '" class="yfb wbtn wbtn-big-transparent ', curInfos.finanTimer, '"><img src="', '/static/common/module/financialsBar/financials_clock.png', '"><span>', curInfos.btnText, '</span></a>');
         }else if(curInfos.buyAble){ 
        _$out_.push('<a href="', curInfos.linkUrl, '" class="qgz wbtn wbtn-big-green">', curInfos.btnText, '</a>');
         }else{ 
        _$out_.push('<a href="', curInfos.linkUrl, '" class="yqw wbtn wbtn-financial-gray">', curInfos.btnText, '</a>');
         } 
        _$out_.push('</div>');
         }else{ /*债权*/ 
        _$out_.push('<div class="financials-item-intro">待收本息：<i>', remainAmount, '</i>元</div><div class="financials-item-intro2">转让价格：');
         if(packStatus == "ZRZ"/*转让中*/){ 
        _$out_.push('<i class="orange">');
         }else if(packStatus == "YJS"/*已转让*/){ 
        _$out_.push('<i class="black">');
         } 
        _$out_.push('', zrAmount, '</i>元</div><div class="financials-item-operate">');
         if(packStatus == "ZRZ"/*转让中*/){ 
        _$out_.push('<a href="javascript:void(0)" class="wbtn wbtn-big-green zqrg" data-zqid="', zrid, '" >立即认购</a>');
         }else if(packStatus == "YJS"/*已转让*/){ 
        _$out_.push('<a href="javascript:void(0)" class="wbtn wbtn-financial-gray" style="line-height: 1.5;cursor: default;">转让时间<br><i>', zrTime, '</i></a>');
         } 
        _$out_.push('</div>');
         } 
        _$out_.push('</div>');
