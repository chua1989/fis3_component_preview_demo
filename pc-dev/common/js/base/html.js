define('common/js/base/html', function(require, exports, module) {

  /**
   *@description 防XSS攻击，针对URL或input获取的数据，用encode进行编译
   * @author 冬冬
   * @date "2016-05-03"
   * 
   *@param {String} n 名称
   *@return {String} hash值
   *@example
   *      mod.encode('<script>hello world</script>');
   */
  ...
  return {
      ...
      encodeBase16forJSON : function(str){
          if (!str) return str;
          str = str.replace(/[\u4E00-\u9FBF]/ig, function(i){return escape(i).replace('%u','\\u')})           
          var _en = [];
          for(var i = 0,len = str.length;i<len;i++){
              _en.push(str.charCodeAt(i).toString(16).toUpperCase());
          }
